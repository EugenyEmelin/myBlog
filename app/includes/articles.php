...
function articles_list() {
	...
	if (isset($_GET['topic']) && $_GET['topic'] != 'all') {
		...
		$sql = "SELECT `id`,`title`,`image`,`article_text`,`topic_id`,`pubdate`,`views` FROM articles WHERE topic_id = :topic ORDER BY `pubdate`";
	} else {
		$sql = "SELECT `id`,`title`,`image`,`article_text`,`topic_id`,`pubdate`,`views` FROM articles ORDER BY `pubdate`";
	}
	$sth = $dbh->prepare($sql);
